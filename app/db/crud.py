...
def update_node_certificate(db: Session, node_id: int, cert_data: dict) -> bool:
    """Store certificate data for a node"""
    try:
        node = db.query(Node).filter(Node.id == node_id).first()
        if not node:
            return False
        
        # Update node with certificate information
        node.certificate = cert_data.get("certificate")
        node.private_key = cert_data.get("private_key")  # Should be encrypted in production
        node.cert_created_at = cert_data.get("created_at")
        node.cert_expires_at = cert_data.get("expires_at")
...
def get_node_certificate(db: Session, node_id: int) -> Optional[dict]:
    """Get certificate data for a node"""
    try:
        node = db.query(Node).filter(Node.id == node_id).first()
        if not node or not node.certificate:
            return None
        
        return {
            "certificate": node.certificate,
            "private_key": node.private_key,
            "created_at": node.cert_created_at,
            "expires_at": node.cert_expires_at
        }
    except Exception:
        return None
...
def store_node_token(db: Session, token_data: dict) -> bool:
    """Store authentication token for a node"""
    try:
        token = NodeToken(
...
def get_node_token(db: Session, node_id: int, token_hash: str) -> Optional[NodeToken]:
    """Get node token by hash"""
    try:
        return db.query(NodeToken).filter(
            NodeToken.node_id == node_id,
            NodeToken.token_hash == token_hash,
            NodeToken.is_active == True
        ).first()
    except Exception:
        return None


def get_node_tokens(db: Session, node_id: int) -> List[NodeToken]:
    """Get all tokens for a node"""
    try:
        return db.query(NodeToken).filter(
            NodeToken.node_id == node_id
        ).order_by(NodeToken.created_at.desc()).all()
    except Exception:
        return []


def update_token_usage(db: Session, token_id: int) -> bool:
    """Update token usage statistics"""
    try:
        token = db.query(NodeToken).filter(NodeToken.id == token_id).first()
        if token:
...
def deactivate_node_token(db: Session, token_id: int) -> bool:
...
def deactivate_node_token_by_hash(db: Session, node_id: int, token_hash: str) -> bool:
    """Deactivate token by hash"""
    try:
        result = db.query(NodeToken).filter(
            NodeToken.node_id == node_id,
            NodeToken.token_hash == token_hash
...
def deactivate_all_node_tokens(db: Session, node_id: int) -> bool:
    """Deactivate all tokens for a node"""
    try:
        result = db.query(NodeToken).filter(
            NodeToken.node_id == node_id
        ).update({"is_active": False})
...
def cleanup_expired_tokens(db: Session) -> int:
    ...
    try:
        result = db.query(NodeToken).filter(
            NodeToken.expires_at < datetime.utcnow()
        ).delete()
...
def record_failed_auth_attempt(db: Session, attempt_data: dict) -> bool:
    """Record failed authentication attempt"""
    try:
        attempt = FailedAuthAttempt(
            node_id=attempt_data["node_id"],
...
def get_failed_auth_attempts(db: Session, node_id: int, since: datetime) -> List[FailedAuthAttempt]:
    """Get failed authentication attempts since a specific time"""
    try:
        return db.query(FailedAuthAttempt).filter(
            FailedAuthAttempt.node_id == node_id,
            FailedAuthAttempt.attempted_at >= since
        ).all()
    except Exception:
        return []


def clear_failed_auth_attempts(db: Session, node_id: int) -> bool:
    """Clear failed authentication attempts for a node"""
    try:
        result = db.query(FailedAuthAttempt).filter(
            FailedAuthAttempt.node_id == node_id
        ).delete()
...
def count_active_tokens(db: Session) -> int:
    """Count active authentication tokens"""
    try:
        return db.query(NodeToken).filter(
            NodeToken.is_active == True,
            NodeToken.expires_at > datetime.utcnow()
...
def count_expired_tokens(db: Session) -> int:
...
def count_failed_attempts_since(db: Session, since: datetime) -> int:
    """Count failed authentication attempts since a specific time"""
    try:
        return db.query(FailedAuthAttempt).filter(
            FailedAuthAttempt.attempted_at >= since
        ).count()
...
def count_locked_out_nodes(db: Session, max_attempts: int, lockout_minutes: int) -> int:
    """Count nodes that are currently locked out"""
    try:
        from datetime import timedelta
        
        cutoff_time = datetime.utcnow() - timedelta(minutes=lockout_minutes)
        
        # Get nodes with too many failed attempts in the lockout window
        subquery = db.query(
            FailedAuthAttempt.node_id,
            func.count(FailedAuthAttempt.id).label('attempt_count')
        ).filter(
            FailedAuthAttempt.attempted_at >= cutoff_time
        ).group_by(FailedAuthAttempt.node_id).subquery()
        
        return db.query(subquery).filter(
            subquery.c.attempt_count >= max_attempts
        ).count()
...
def get_last_token_cleanup(db: Session) -> Optional[datetime]:
    """Get timestamp of last token cleanup"""
    try:
        # This could be stored in a system settings table
        # For now, just return None - could be implemented with a settings table
        return None
    except Exception:
        return None
