...
def compile_proto_from_source(dist):
    ...
    command = (
        ...
        f"-I={v2ray_dir} "
        f"--python_out={dist} "
        f"--grpclib_python_out={dist} " + proto_files
    )
...
def fix_proto_imports(dist):
    ...
    # new_imp = f"wildosnode.{parent_dir.name}.{curr_dir.name}." + "{}"
            ...
            if file.endswith(".py"):
                ...
                with open(path, "r") as pyfile:
                    ...
                    for imp in imports:
                        content = re.sub(
                            rf"^([^'\r\n]*){imp}([\s\.].*)$",
                            rf"\1wildosnode.backends.xray.{parent_dir.name}.{curr_dir.name}.{imp}\2",
                            content,
                            flags=re.MULTILINE,
                        )
                        content = re.sub(
                            rf"^(.*importlib\.import_module\(\'){imp}(.*)$",
                            rf"\1wildosnode.backends.xray.{parent_dir.name}.{curr_dir.name}.{imp}\2",
                            content,
                            flags=re.MULTILINE,
                        )
