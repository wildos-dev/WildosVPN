...
# Download Xray latest for WildosVPN
# Based on original script with improvements for WildosVPN
...
check_if_running_as_root() {
    # Skip root check in Docker environment
    if [[ -f /.dockerenv ]]; then
        return 0
    fi
...
place_xray() {
    ...
    echo "Xray files installed successfully"
}
...
main() {
    ...
    if command -v xray >/dev/null 2>&1; then
        echo "WildosVPN: Xray installation completed successfully"
        xray version | head -n1
    else
        echo "WildosVPN: Error - Xray installation failed"
        exit 1
    fi
}
